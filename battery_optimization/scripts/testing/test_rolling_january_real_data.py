"""
Test rolling horizon optimizer for January 2024 with REAL DATA (1-hour resolution).
...
def test_rolling_january_real_data():
    """Simulate January 2024 with rolling horizon optimization using REAL data (1-hour resolution)."""

    print("\n" + "="*80)
    print("ROLLING HORIZON SIMULATION - JANUARY 2024 (REAL DATA, 1-HOUR RESOLUTION)")
    print("="*80)
    ...
    # Extract January 2024 (31 days × 24 hours = 744 timesteps)
    start_date = pd.Timestamp(2024, 1, 1, 0, 0, tz='Europe/Oslo')
    end_date = pd.Timestamp(2024, 1, 31, 23, 0, tz='Europe/Oslo')
    price_january = price_df_hourly.loc[start_date:end_date]

    spot_prices = price_january['price_nok'].values
    timestamps = price_january.index
    ...
    # For each hour in January 2024, find corresponding hour in PVGIS data (by day-of-year and hour)
    ...
    print(f"  Average production: {pv_production.mean():.1f} kW (winter = low solar)")
    ...
    # Generate hourly consumption for January 2024
    ...
    # ========================================================================
    print(f"\n" + "="*80)
    print("JANUARY 2024 SIMULATION RESULTS (REAL DATA)")
    print("="*80)
    ...
    if results:
        ...
        print(f"  Degradation cost: {df['degradation_cost'].sum():,.2f} NOK")
        print(f"  Power tariff (monthly): {monthly_tariff_actual:,.2f} NOK")
        total_cost = df['energy_cost'].sum() + df['degradation_cost'].sum() + monthly_tariff_actual
        ...
        print(f"  Total equivalent cycles: {df['equivalent_cycles'].sum():.2f} cycles")
        ...
        ax.set_title('January 2024: Power Flows (REAL DATA)')
        ...
        plt.tight_layout()
        plot_path = '/mnt/c/Users/klaus/klauspython/SDE/battery_optimization/results/january_2024_real_data.png'
        ...
        ax.axhline(y=max_peak_month, color='red', linestyle='--', label=f'Monthly Peak ({max_peak_month:.1f} kW)')
        ax.set_ylabel('Power [kW]')
        ax.set_title('January 1-7, 2024: Power Flows (Detailed)')
        ax.legend(loc='upper right')
        ax.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plot_zoom_path = '/mnt/c/Users/klaus/klauspython/SDE/battery_optimization/results/january_2024_real_data_zoom.png'
...
if __name__ == "__main__":
    import sys
    success = test_rolling_january_real_data()

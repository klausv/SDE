"""
Test rolling horizon optimizer for May 2024 with REAL DATA (1-hour resolution).
...
def test_rolling_may_real_data():
    """Simulate May 2024 with rolling horizon optimization using REAL data (1-hour resolution)."""

    print("\n" + "="*80)
    print("ROLLING HORIZON SIMULATION - MAY 2024 (REAL DATA, 1-HOUR RESOLUTION)")
    print("="*80)
    ...
    # Extract May 2024 (31 days × 24 hours = 744 timesteps)
    start_date = pd.Timestamp(2024, 5, 1, 0, 0, tz='Europe/Oslo')
    end_date = pd.Timestamp(2024, 5, 31, 23, 0, tz='Europe/Oslo')
    price_may = price_df_hourly.loc[start_date:end_date]

    spot_prices = price_may['price_nok'].values
    timestamps = price_may.index
    ...
    # For each hour in May 2024, find corresponding hour in PVGIS data (by day-of-year and hour)
    ...
    print(f"  Average production: {pv_production.mean():.1f} kW (spring = medium-high solar)")
    ...
    # Generate hourly consumption for May 2024
    ...
    # ========================================================================
    print(f"\n" + "="*80)
    print("MAY 2024 SIMULATION RESULTS (REAL DATA)")
    print("="*80)
    ...
    if results:
        ...
        ax.set_title('May 2024: Power Flows (REAL DATA)')
        ...
        plt.tight_layout()
        plot_path = '/mnt/c/Users/klaus/klauspython/SDE/battery_optimization/results/may_2024_real_data.png'
        ...
        ax.axhline(y=max_peak_month, color='red', linestyle='--', label=f'Monthly Peak ({max_peak_month:.1f} kW)')
        ax.set_ylabel('Power [kW]')
        ax.set_title('May 1-7, 2024: Power Flows (Detailed)')
        ax.legend(loc='upper right')
        ax.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plot_zoom_path = '/mnt/c/Users/klaus/klauspython/SDE/battery_optimization/results/may_2024_real_data_zoom.png'
...
if __name__ == "__main__":
    import sys
    success = test_rolling_may_real_data()

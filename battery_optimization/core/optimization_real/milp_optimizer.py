...
class MILPBatteryOptimizer:
    ...
    def optimize_with_ortools(
        ...
        grid_export = {}
        curtailment = {}
        charging = {}  # Binary variable for charge/discharge mutual exclusion
        grid_importing = {}  # Binary variable: 1 if importing, 0 if exporting
        ...
        for d in range(self.n_typical_days):
            for h in range(24):
                ...
                charging[t] = solver.IntVar(0, 1, f'charging_{t}')  # Binary: 1 if charging, 0 if discharging
                grid_importing[t] = solver.IntVar(0, 1, f'grid_importing_{t}')  # Binary: 1 if importing
        ...
        # 1. C-rate constraints (link power and capacity)
        solver.Add(battery_kw <= battery_kwh * 1.0)  # Max C-rate = 1.0C
        solver.Add(battery_kw >= battery_kwh * 0.25)  # Min C-rate = 0.25C

        # 2. Battery sizing and mutual exclusion constraints
        for t in range(self.n_timesteps):
            ...
            solver.Add(charge[t] <= battery_kw * charging[t])
            solver.Add(discharge[t] <= battery_kw * (1 - charging[t]))

            # SOC limited by capacity
            solver.Add(soc[t] <= battery_kwh * 0.9)  # Max SOC
            solver.Add(soc[t] >= battery_kwh * 0.1)  # Min SOC

            # Import/Export mutual exclusion and capacity limits
            # Use grid connection capacity as symmetric limit
            Gmax = self.system_config.grid_capacity_kw
            solver.Add(grid_import[t] <= Gmax * grid_importing[t])
            solver.Add(grid_export[t] <= Gmax * (1 - grid_importing[t]))
        ...
        # 3. Cyclic SOC constraint (battery must end where it started each day)
        for d in range(self.n_typical_days):
            start_t = d * 24
            end_t = d * 24 + 23
            solver.Add(soc[end_t] == soc[start_t])

        # 4. (Removed) Minimum cycles constraint – avoid forcing uneconomic usage

        # 4b. Maximum daily depth of discharge (optional longevity constraint)
        max_daily_dod = 0.8  # Max 80% DOD per day
        for d in range(self.n_typical_days):
            daily_discharge = solver.Sum([discharge[t] for t in range(d*24, (d+1)*24)])
            solver.Add(daily_discharge <= battery_kwh * max_daily_dod)
        ...
        # Revenue and costs
        # Arbitrage based on grid import/export flows
        arbitrage = 0
        for t in range(self.n_timesteps):
            d = t // 24
            h = t % 24
            price = price_profiles[d, h]
            arbitrage += (- price * grid_import[t] + price * grid_export[t])
        arbitrage *= annual_factor

        # Monthly power tariff cost (one typical day per month)
        tariff_values = [136, 232, 372, 572, 772, 972, 1772, 2572, 3372, 5600]
        annual_power_tariff_cost = sum(
            sum(tariff_tier[month][i] * tariff_values[i] for i in range(len(tier_limits)))
            for month in range(12)
        )

        # Curtailment is a lost opportunity (cost)
        curtailment_cost = 0
        for t in range(self.n_timesteps):
            d = t // 24
            h = t % 24
            curtailment_cost += curtailment[t] * price_profiles[d, h]
        curtailment_cost *= annual_factor

        # Total annual cash flow (positive means revenue)
        annual_cashflow = arbitrage - annual_power_tariff_cost - curtailment_cost

        # NPV over lifetime (simplified)
        npv = 0
        discount = 1.0
        for year in range(self.economic_config.battery_lifetime_years):
            npv += annual_cashflow * discount * (1 - 0.02 * year)  # Degradation
